    ...
    const fetchMoviesByLanguage = async () => {
      try {
        const res = await axios.get(
          `https://api.themoviedb.org/3/discover/movie?api_key=${process.env.REACT_APP_TMDB_API_KEY}&with_original_language=${langCode}&sort_by=popularity.desc`
        );
        setMovies(res.data.results);
      } catch (error) {
        console.error("Failed to fetch movies by language:", error);
      }
    };
  ...
  return (
    <div className="p-6">
      <h1 className="mb-4 text-3xl font-bold text-white">
        Top Movies in Language: {langCode.toUpperCase()}
      </h1>
